...
async def add_hall_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        hall_photos = data['p_photos'] #TODOPROJECT # Фото сохранены в строке, разделенной запятыми
...
@admin_private.callback_query(F.data == 'delete_foods')
async def delete_foods(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(Admin.d_item)
    items = await sync_to_async(Foods.objects.all)()
    if items:
        item_list = "\n".join([f"{item.id}. {item.name}, {item.price} руб." for item in items])
        await callback.message.answer(f"Категория 'Еда':\n{item_list}\n\n👉 Введите номер элемента, который хотите удалить", reply_markup=kb.offer_cancel())
    else:
        await callback.message.answer("В категории 'Еда' нет элементов для удаления.", reply_markup=kb.admin_panel())
        await state.clear()
@admin_private.callback_query(F.data == 'delete_services')
async def delete_foods(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(Admin.d_item)
    items = await sync_to_async(Foods.objects.all)()
...
@admin_private.message(Admin.d_item)
async def process_delete_item(message: Message, state: FSMContext):
    item_id = message.text
    data = await state.get_data()
    type_for = data.get('type_for')
    
    if type_for == 1:
        deleted = await delete_item(Foods, item_id)
    elif type_for == 2:
        deleted = await delete_item(Services, item_id)
    elif type_for == 3:
        deleted = await delete_item(Goods, item_id)
    elif type_for == 4:
        deleted = await delete_item(Halls, item_id)
    
    if deleted:
        await message.answer(f"Элемент с номером {item_id} успешно удалён", reply_markup=kb.admin_panel())
    else:
        await message.answer(f"Элемент с номером {item_id} не найден", reply_markup=kb.offer_cancel())
