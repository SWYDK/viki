...
from .models import Booked, Notify, WebAppData
...
def delete_webdata():
    WebAppData.objects.filter(is_viewed=True).delete()
...
def send_telegram_message(user, hall_name, book_time, reminder_type):
    ...
    # Проверяем наличие записи с таким же tg_id и msg
    if not Notify.objects.filter(tg_id=user.tg_id, msg=text).exists():
...
def check_reminders():
    ...
    for booking in bookings:
        ...
        if time_until_booking <= timedelta(hours=1) and time_until_booking > timedelta(minutes=59):
            ...
            if booking.reminder_sent_hour == False:
        ...
        # Если осталось ровно 1 день
            ...
            if booking.reminder_sent_day == False:
        ...
        # Если осталось ровно 1 неделя
            ...
            if booking.reminder_sent_week == False:
